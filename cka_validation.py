...
def test_activation_extraction():
    ...
    try:
        ...
        # Test CKA with same layer (should be 1.0)
        cka_score_same = linear_cka(activations_1, activations_1)
        logger.info(f"✓ CKA between layer 1 and itself: {cka_score_same:.6f}")
        
        # Test CKA with different activations from same layer
        activations_1_2 = get_layer_activations(model, test_loader, '1', device)
        cka_score_diff = linear_cka(activations_1, activations_1_2)
        logger.info(f"✓ CKA between two runs of layer 1: {cka_score_diff:.6f}")
